                    ...
                    int rings)
{
  ...
  PointCloud<pcl::PointXYZI>::Ptr tmp_cloud_ptr (new PointCloud<pcl::PointXYZI>);
  ...
  //std::cout << "points.size: " << cloud_XYZIR->points.size() << std::endl;
  for (int i=0; i < cloud_XYZIR->points.size(); i++)
  {
    ring = cloud_XYZIR->points[i].ring;
    //std::cout << "ring number: " << std::to_string(ring) << std::endl;
  ...
  for (int col=0; col < row_count[0]; col++)
  {
    ...
    {
      //std::cout << tmp_cloud_ptr->at(col, row);
