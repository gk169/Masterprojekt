...
def echoTopic():
    ...
    msg_img = None
    msg_cam = None
    for topic, msg, t in bag.read_messages(topics=['/velodyne/front/velodyne_points', '/velodyne/top/velodyne_points', '/usb_cam/image_raw/compressed', '/usb_cam/camera_info']):
        #print(msg)
        print(topic)
        print(t)
        ...
        if topic == '/usb_cam/image_raw/compressed' and x_img < 1:
            x_img=x_img+1
            msg_img = msg
        if topic == '/usb_cam/camera_info' and x_cam < 1:
            x_cam=x_cam+1
            msg_cam = msg
        if x_front==1 and x_top==1 and x_img==1 and x_cam==1:
            break
